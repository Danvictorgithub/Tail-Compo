...
export class EmailService {
    constructor(private db: PrismaService, private mailService: MailService, private usersService: UsersService) { }
    ...
    async createPasswordReset(passwordResetDto: PasswordResetDto) {
        const user = await this.db.user.findUnique({ where: { email: passwordResetDto.email } })
        if (!user || user.emailVerified) {
            return { message: "Request queued" }
        }
        await this.db.emailToken.deleteMany({ where: { userId: user.id, type: 'RESET_PASSWORD' } })
        const emailToken = await this.db.emailToken.create({
            data: {
                userId: user.id,
                type: 'RESET_PASSWORD',
    ...
    async verifyPasswordReset(id: string, passwordResetVerifyDto: PasswordResetVerifyDto, user: User) {
        try {
            console.log('this is successful 0')
            await this.getPasswordReset(id, user);
            console.log('this successful')
            await this.usersService.update(user.id, { password: passwordResetVerifyDto.password });
