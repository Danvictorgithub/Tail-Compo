...
export class AuthService {
    ...
    async login(user: User) {
        ...
        const payload = { email: user.email, sub: user.id, name: profile.name, image: profile.image };
        return {
    ...
    async googleAuthentication(payload: GoogleOAuthPayload) {
        ...
        if (!user) {
            const newUser = await this.userService.createVerified({
                email: payload.email,
                username: await this.createUsername(`${payload.firstName}${payload.lastName}`.trim()),
